...
In the Host Name field, type hadoop@ec2-***.compute-1.amazonaws.com
...
ssh -i ~/ec2bastion_keypair.pem -ND 8157 hadoop@ec2-***.compute-1.amazonaws.com
...
4. In the Host Name field, enter hadoop@ec2-***.compute-1.amazonaws.com
...
# Fix the issue: 
# >> ec2_user@ec2-***.compute-1.amazonaws.com: Permission denied (publickey,gssapi-keyex,gssapi-with-mic).
# sudo su
...
## ssh ec2-user@ec2-***.compute-1.amazonaws.com -L 2222:DESTINATION-EC2-IP:22
 ...
 # new Bastion:
 IP: ec2***.comp bastion only for my ip 99.93.13.10
	chmod 400 ec2bastion_keypair.pem
	## ssh -i "ec2bastionpem.pem" ec2-user@ec2-***.compute-1.amazonaws.com -L 2222:ec2-***.compute-1.amazonaws.com:22
...
Example:
	ssh -i "ec2bastionpem.pem" ec2-user@ec2-***.compute-1.amazonaws.com	
  -Then connect to Flink cluster: Private IP/Hostname type: ip-***.ec2.internal

Flink Public IP: ***  DNS:  ec2-***.compute-1.amazonaws.com
  
		ssh ec2-user@ec2-***.compute-1.amazonaws.com -L 2222:ec2-***.compute-1.amazonaws.com:22
  ...
  -Then upload file  Open separate SSH:
	...
	# Flink IP: ***
	ssh -i "ec2bastionpem.pem" ec2-user@ec2-***.compute-1.amazonaws.com -L 2222:ec2-***.compute-1.amazonaws.com:22
   ## scp -i ~/c/Lana/.aws/ec2bastionpem.pem ~/Desktop/MS115.fa  ubuntu@ec2-***.compute-1.amazonaws.com:~/data/
   ...
   ## Download: 
	scp -i ~/Desktop/amazon.pem ubuntu@ec2-***.compute-1.amazonaws.com:/data/ecoli_ref-5m-trim.fastq.gz ~/Download/
	
	## With winCSP
	-bastion: ip*** "ec2ppk-key" ec2-***.compute-1.amazonaws.com
	...
	Destination port: Input <Internal IP Address of EC2 Machine flink-cluster: ip***>:22 and click Add.
	
	ec2-ip***compute-1.amazonaws.com ip***
	
>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# Flink flink-ec2cluster ->  i-031cb6aca6e974665  Private: ip-***.ec2.internal  
>> PUBLIC: ec2-***.compute-1.amazonaws.com

chmod 400 ~/.aws/ec2bastion_keypair.pem
ssh -i "ec2bastion_keypair.pem" ec2-user@ec2-***.compute-1.amazonaws.com:22
-i ~/.aws/ec2bastion_keypair.pem private
# BAD -> ssh -i "ec2bastion_keypair.pem" ec2-user@ec2-***.compute-1.amazonaws.com  -L 2222:ip-***.ec2.internal:22
## IP: 
***
ssh -i "ec2bastion_keypair.pem" ec2-user@ec2-***.compute-1.amazonaws.com  -L 2222:ec2-***.compute-1.amazonaws.com:22
## transfer the file:
	scp -P 2222 Source-File-Path user-fqdn@localhost:
## Transfer folder:
	scp -r -P 2222 flink-1.16.1 ec2-user@ec2-***.compute-1.amazonaws.com:22
<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

# Small Mem... i-03e98ff23030e8703 (flink-cluster)->   ec2-***.compute-1.amazonaws.com   private: ip-***.ec2.internal
# Flink-ec2 Public IPv4 DNSPublic IPv4 DNS c5alarge =2x4Gb = 0.07  IP name: ip-***.ec2.internal
ssh -i "ec2bastion_keypair.pem" ec2_user@ec2-***.compute-1.amazonaws.com -L 2222 ec2_user@ec2-***.compute-1.amazonaws.com:22  
>>>>>>
# i-08ce88847e4fa58c6 ecbastionpem   ->   ec2-***.compute-1.amazonaws.com            private: ip-***.ec2.internal
# i-0c08dd71651da9ffa (bastionppk)   ->   ec2-***.compute-1.amazonaws.com  private: *** ip-***.ec2.internal
# i-06fa5d7f2adefa2db (bastion-2ecbkey)-> ec2-***.compute-1.amazonaws.com  private: ip-***.ec2.internal

Connect to Flink SSH ->
 ssh -i "ec2bastionpem.pem" ec2-user@ec2-***.compute-1.amazonaws.com -L 2222:ec2-***.compute-1.amazonaws.com:22
 ssh -i "ec2bastionpem.pem" ec2-user@ec2-***.compute-1.amazonaws.com:22

 ssh -i ec2bastion_keypair.pem ec2-***.compute-1.amazonaws.com  -L 2222:***.compute-1.amazonaws.com:22
 ==============================================================================================================
 - 1. Add docker to ec2:
 sudo yum update
...
### Connect to other ssh:
	ssh -i "/C/Users/Lana/.aws/ec2bastion_keypair.pem" ec2-user@ec2-****.compute-1.amazonaws.com
	#
	docker run \
    --rm \
    --name=taskmanager \
	...
	>> [java.net.ConnectException: Connection refused: cdf9ab930654/***:6123]
